    ...
    try:
        ...
        while get_current_ist_time().strftime("%H:%M") < "14:59":
            ...
            if not ce_updated and ce_ltp >= ce_entry_price * 1.20:
                log_and_print("NIFTY:🚀 CE +20% → attempting SL upgrade to 40%")
                # Check if 30% SL already hit before we can cancel
                # This prevents placing a new SL on an already-closed position
                if is_order_executed(ce_sl_order_1043):
                    log_and_print("NIFTY:⚠️ CE 30% SL already executed — skipping upgrade, position closed")
                    ce_updated = True
                    ce_sl_logged = True
                    trade_logger.log_sl_hit("CE")
                    send_telegram("NIFTY:⚠️ CE 30% SL hit before upgrade could happen. Position closed at 30%.")
                else:
                    ...
                    if new_ce_sl_order:
                        ...
                        log_and_print(f"NIFTY: CE SL upgraded → new order {ce_sl_order_1043}")
                        send_telegram("NIFTY: CE SL upgraded to 40%. PLEASE VERIFY.")
                    else:
                        log_and_print("NIFTY:❌ CE 40% SL placement FAILED — no active SL on CE!")
                        send_telegram("NIFTY:❌ CE 40% SL placement FAILED. PLACE MANUALLY IMMEDIATELY!")
                    ce_updated = True
            ...
            if not pe_updated and pe_ltp >= pe_entry_price * 1.20:
                log_and_print("NIFTY:🚀 PE +20% → attempting SL upgrade to 40%")
                # Check if 30% SL already hit before we can cancel
                if is_order_executed(pe_sl_order_1043):
                    log_and_print("NIFTY:⚠️ PE 30% SL already executed — skipping upgrade, position closed")
                    pe_updated = True
                    pe_sl_logged = True
                    trade_logger.log_sl_hit("PE")
                    send_telegram("NIFTY:⚠️ PE 30% SL hit before upgrade could happen. Position closed at 30%.")
                else:
                    ...
                    if new_pe_sl_order:
                        ...
                        log_and_print(f"NIFTY: PE SL upgraded → new order {pe_sl_order_1043}")
                        send_telegram("NIFTY: PE SL upgraded to 40%. PLEASE VERIFY.")
                    else:
                        log_and_print("NIFTY:❌ PE 40% SL placement FAILED — no active SL on PE!")
                        send_telegram("NIFTY:❌ PE 40% SL placement FAILED. PLACE MANUALLY IMMEDIATELY!")
                    pe_updated = True
        ...
        if not ce_sl_logged:
            ...
            log_and_print("NIFTY: CE already closed by SL hit — skipping square off")

        if not pe_sl_logged:
            square_off(pe_symbol, pe_token, pe_sl_order_1043)
        else:
            log_and_print("NIFTY: PE already closed by SL hit — skipping square off")
        if reentered:
